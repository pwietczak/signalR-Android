...
class MessageUiPreviewDataProvider: PreviewParameterProvider<MessageUi> {
	override val values = sequenceOf(
		...
		MessageUi(
			time = LocalDateTime(2016, 2, 15, 16, 57, 0, 0),
			text= "This is a MessageUi"
		       ),

		// 2. Bardzo długa wiadomość
		MessageUi(
			time = LocalDateTime(2024, 12, 31, 23, 59, 59, 999_999_999),
		...
		// 3. Pusta wiadomość
		MessageUi(
			time = LocalDateTime(2023, 1, 1, 0, 0, 0, 0),
			text = ""
		       ),

		// 4. Same spacje
		MessageUi(
			time = LocalDateTime(2023, 6, 10, 12, 30, 0, 0),
			text = "     "
		       ),

		// 5. Emoji i Unicode
		MessageUi(
			time = LocalDateTime(2025, 5, 20, 8, 15, 0, 0),
			text = "🔥🚀 Jetpack Compose is awesome 😎✨"
		       ),

		// 6. Jeden znak
		MessageUi(
			time = LocalDateTime(2022, 9, 9, 9, 9, 9, 9),
			text = "👍"
		       ),

		// 7. Wielolinijkowa wiadomość
		MessageUi(
			time = LocalDateTime(2024, 3, 14, 1, 59, 26, 0),
			text = "Line 1\nLine 2\nLine 3\nLine 4"
		       ),

		// 8. URL w treści
		MessageUi(
			time = LocalDateTime(2025, 1, 5, 18, 45, 0, 0),
			text = "Check this out: https://developer.android.com/jetpack/compose"
		       ),

		// 9. Epoch (symbolicznie)
		MessageUi(
			time = LocalDateTime(1970, 1, 1, 0, 0, 0, 0),
			text = "Unix epoch start"
		       ),

		// 10. Daleka przyszłość
		MessageUi(
			time = LocalDateTime(2099, 12, 31, 23, 59, 59, 0),
